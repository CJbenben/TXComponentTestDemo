...
static const CGFloat CSToastVerticalPadding     = 7.0;
static const CGFloat CSToastCornerRadius        = 15.0;
...
static const BOOL    CSToastDisplayShadow       = NO;//YES
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        else if([point caseInsensitiveCompare:@"custom"] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 - CSToastVerticalPadding * 5);
        }
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    wrapperView.backgroundColor = [[UIColor colorWithRed:224/255.0 green:224/255.0 blue:224/255.0 alpha:1] colorWithAlphaComponent:CSToastOpacity];
...
//        CGSize expectedSizeTitle = [title sizeWithFont:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode];
        CGSize expectedSizeTitle = [self sizeWithStr:title font:titleLabel.font maxSize:maxSizeTitle];//[title sizeWithFont:titleLabel.font maxSize:maxSizeTitle];
    ...
    if (message != nil) {
        ...
        messageLabel.textColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1];
...
//        CGSize expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        CGSize expectedSizeMessage = [self sizeWithStr:message font:messageLabel.font maxSize:maxSizeMessage];//[message sizeWithFont:messageLabel.font maxSize:maxSizeMessage];
...
- (CGSize)sizeWithStr:(NSString *)str font:(UIFont *)font maxSize:(CGSize)maxSize {
